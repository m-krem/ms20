---
layout: page
title: À propos
permalink: /about/
---

## Qu'est-ce que le MS20 ?

Le **MS20** est le makerspace n°20 du Luxembourg. C’est un espace dédié à la **création**, à l’**innovation** et au **partage**, ouvert aux élèves, enseignants et membres de la communauté éducative de [Vauban, École et Lycée français de Luxembourg](https://vauban.lu).

Les makerspaces du Luxembourg sont une initiative de [BEE Creative](https://makerspace.lu). Dans ce cadre, le MS20 a été créé en **2018** sous l'impulsion de Monsieur Metzler, professeur agrégé de Physique-Chimie et de NSI, à la suite du rôle de l’établissement comme hôte de la **Makerfest**, l’événement annuel désormais organisé par [base1](https://base1.lu).

---

## Ce que l’on peut y faire

- Prototypage et fabrication numérique  
- Programmation, électronique et robotique  
- Impression 3D, découpe laser, fabrication artisanale  
- Projets collaboratifs et interdisciplinaires  
- Expérimentation et apprentissage par la pratique  

Le makerspace encourage l’**autonomie**, la **curiosité** et le **travail en équipe**.

---

## Une philosophie

MS20 s’inscrit dans la culture **maker** :
> apprendre en faisant, tester, se tromper, recommencer et partager.

---

## Contact

Pour toute question ou proposition de projet :
<ul class="social-media-list">
      <li>
        <svg class="svg-emoji"><use xlink:href="{{ '/assets/emojis/e-mail.svg' | relative_url }}"></use></svg><a class="u-email" href="mailto:{{ site.email }}">{{ site.email }}</a>
      </li>
</ul>

---

Le code source de ce site est disponible sur GitHub : [m-krem](https://github.com/m-krem) / [ms20](https://github.com/m-krem/ms20)
